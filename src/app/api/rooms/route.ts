...
export async function POST(req: NextRequest) {
  try {
    ...
    // train_number unique constraint 충돌 시 기존 row를 반환
    const { data: room, error: roomError } = await db
      .from('rooms')
      .upsert(
        { train_number: trainNumber, status: 'waiting' },
        { onConflict: 'train_number', ignoreDuplicates: false }
      )
      .select()
      .single();

    if (roomError || !room) {
      // upsert 실패 시 기존 방 조회로 fallback
      ...
      if (!fallbackRoom) {
        throw roomError ?? new Error('방을 찾을 수 없습니다.');
      }

      return handlePlayer(db, fallbackRoom, fingerprint);
    }

    return handlePlayer(db, room, fingerprint);
  } catch (err) {
...
async function handlePlayer(db: any, room: any, fingerprint: string) {
